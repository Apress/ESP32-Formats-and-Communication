...
void setup()
{
  ...
  battDesc = new BLEDescriptor((uint16_t)0x2901);
  battDesc->setValue("Battery level text");  // user defined description
  battNotfy = new BLE2902();
  battNotfy->setNotifications(true);  // notification automatically on
  battChar.addDescriptor(battDesc);     // add descriptor to characteristic
  battChar.addDescriptor(battNotfy);  // link notifier to descriptor
  battService->start();
  ...
  tempNotfy = (BLE2902*)tempDesc;  // link notifier to descriptor
  tempChar.addDescriptor(tempDesc);  // add descriptor to characteristic
  tempService->start();
  lag = 0;
